...
app.post("/add-listing", async (req, res) => {
  try {
    ...
    await pool.query(
      "INSERT INTO saved_listings (name, profit, demand, risk) VALUES ($1, $2, $3, $4)",
...
Bu ürünü e-ticaret açısından kısaca analiz et.
...
`.trim();
    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "Sen e-ticaret ve dropshipping konusunda kısa, net analiz yapan bir uzmansın."
          },
          {
            role: "user",
            content: prompt
    ...
    if (!response.ok) {
      return res.status(500).json({
        error: data.error?.message || "OpenAI isteği başarısız"
      });
    }

    const result = data.choices?.[0]?.message?.content || "Analiz üretilemedi.";
    res.json({ result });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
